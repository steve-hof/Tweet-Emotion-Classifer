...
hyp_str = "twtLen_" + str(MAX_BLURB_LENGTH) + "nLST_" + str(LSTM_UNITS) + "lr_" + str(LEARNING_RATE) + "n_hid_" + "_"


def _getTrainBatch(train_emo_ids, train_no_emo_ids):
    labels = []
...
def _getTestBatch(test_emo_ids, test_no_emo_ids):
    labels = []
    arr = np.zeros([BATCH_SIZE, MAX_BLURB_LENGTH])
    for i in range(BATCH_SIZE):
        if i % 2 == 0:
...
def main():
    ...
    with open('tweet_data.pickle', 'rb') as f:
    ...
    for i in range(ITERATIONS):
        ...
        next_train_batch, next_train_batchLabels = _getTrainBatch(train_emo, train_no_emo);
        sess.run(optimizer, {input_data: next_train_batch, labels: next_train_batchLabels})

        # Write training summary to board
        if i % 100 == 0:
            summary = sess.run(summary_op, {input_data: next_train_batch, labels: next_train_batchLabels})
            train_summary_writer.add_summary(summary, i)
            train_summary_writer.flush()

            next_test_batch, next_test_batchLabels = _getTestBatch(test_emo, test_no_emo);
            testSummary = sess.run(summary_op, {input_data: next_test_batch, labels: next_test_batchLabels})
